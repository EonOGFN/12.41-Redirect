...
namespace li { namespace detail {
    ...
    {
        // Convert the character to lowercase if CaseSensitive is false
        unsigned adjusted_char = (!CaseSensitive && c >= 'A' && c <= 'Z') ? (c | 0x20) : c;
        return (value ^ adjusted_char) * 131 + (adjusted_char << 5) + (adjusted_char >> 3);
    }
...
#if defined(_MSC_VER)
        static const win::PEB_T* peb = reinterpret_cast<const win::PEB_T*>(__readgsqword(0x60));
